module FillerTool
  ...
  def FillerTool.findFillers(duration, min_candidate, max_candidate, allowed_gap, allow_duplicate)
  ...
  def FillerTool.findFillerData(duration, min_candidate, max_candidate, allowed_gap, allow_duplicate)
  ...
  def FillerTool.addCandidate(candidates, newCandidates, index)
    num = newCandidates.size
    candidates[index] = newCandidates if num > 0
    return num
  end

  def FillerTool.invalid_param(name)
    [nil, "Please set proper value for parameter '#{name}'", 400]
  end

  def FillerTool.durStr(dur)
    "%d:%02d" % [dur/60, dur%60]
  end
end
